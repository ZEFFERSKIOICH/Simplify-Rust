...
fn query(l:usize,pos:usize,r:usize,seg:&Vec<i64>,ql:usize,qr:usize)->i64{
   if l>qr || r<ql      {return 0i64;}
   if l>=ql && r<=qr    {return seg[pos];}
   let midi:usize=l+((r-l)>>1);
   return query(l,pos<<1,midi,seg,ql,qr)+query(midi+1,pos<<1|1,r,seg,ql,qr);
}
...
fn main(){
    ...
    for _queries in 0..q{
        ...
        if ch%2==1{
            ...
            ql-=1;qr-=1;
            println!("{}",query(0usize,1usize,n-1,&seg,ql,qr));
        }
        else{
            let mut pos:usize=cin();pos-=1;
            let val:i64=cin();
            update(0usize,1usize,n-1,&mut seg,pos,val);
        }
    }
